              ...
              {data.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {data.map((item, idx) => {
                    ...
                    let displayTitle = "Hero Slide";
                    try {
                      if (item.h1) {
                        const parsed = (typeof item.h1 === 'string' && item.h1.startsWith('[')) ? JSON.parse(item.h1) : item.h1;
                        displayTitle = Array.isArray(parsed) ? parsed.join(' ') : parsed;
                      }
                    } catch (e) {
                      displayTitle = item.h1 || "Hero Slide";
                    }
                      ...
                      <div key={item.id || idx} className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-lg transition-all group">
                        <div className="relative h-44 bg-gray-100 overflow-hidden">
                          ...
                          <div className="absolute bottom-3 left-4">
                            <span className="text-white text-[10px] font-black uppercase tracking-widest bg-white/20 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/30">
                              {item.tag || 'Promotional'}
                            </span>
                          </div>
                          <div className="absolute top-3 right-3 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                              onClick={() => { setFormData({...item}); setEditingId(item.id); setViewMode('form'); }}
                              className="p-2.5 bg-white rounded-xl shadow-lg text-blue hover:bg-blue hover:text-white transition-all"
                            ><Edit3 size={14} /></button>
                            <button
                              onClick={() => handleDelete(item.id)}
                              className="p-2.5 bg-white rounded-xl shadow-lg text-red-500 hover:bg-red-500 hover:text-white transition-all"
                            ><Trash2 size={14} /></button>
                          </div>
                        </div>
                        <div className="p-5">
                          <h4 className="font-bold text-ink text-sm mb-1 truncate">{displayTitle}</h4>
                          <p className="text-[11px] text-muted line-clamp-2 mb-4 leading-relaxed">{item.description}</p>
                          <div className="flex gap-2">
                            <button
                              onClick={() => { setFormData({...item}); setEditingId(item.id); setViewMode('form'); }}
                              className="flex-1 flex items-center justify-center gap-2 py-3 bg-blue/5 text-blue rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-blue hover:text-white transition-all"
